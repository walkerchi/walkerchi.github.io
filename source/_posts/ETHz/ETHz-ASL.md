...
1. We are compute-bound, since we have $O(n^2+nm)$ operations to be performed on $O(n+m)$ data, n being the number of pedestrians, m the number of obstacles.
...
which is a reduction by ca. $1 \text{flop/cycle}$ compared to our previous performance bound.
...
### Experimental Setup
All benchmarks were performed on an Intel Core i7-7500U (Kaby Lake), clocked at $2.7 \text{GHz}$ (no Turbo Boost), using a single core. Kaby Lake is microarchitecturally almost identical to Skylake.
